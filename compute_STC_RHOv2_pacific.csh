...
!!let RHOLEVEL_TRANSRHO9N     = TRANSRHO9N[z=1031:1038@loc:0]
!!let RHOLEVEL_TRANSRHO9S     = TRANSRHO9S[z=1033:1038@loc:0]
!!let RHOLEVEL_TRANSRHO9N_INT = TRANSRHO9N_INT[z=1031:1038@loc:0]
...
let transrho9n_above_rholevel     = if(Z LT RHOLEVEL_TRANSRHO9N) then TRANSRHO9N else 0.
let transrho9s_above_rholevel     = if(Z LT RHOLEVEL_TRANSRHO9S) then TRANSRHO9S else 0.
let transrho9n_int_above_rholevel = if(Z LT RHOLEVEL_TRANSRHO9N_INT) then TRANSRHO9N_INT else 0.
let transrho9s_int_above_rholevel = if(Z LT RHOLEVEL_TRANSRHO9S_INT) then TRANSRHO9S_INT else 0.
...
save/file=STC_RHO_v2_${EXP}.nc/clobber CONVRHO[k=@sum],CONVRHO_INT[k=@sum]
save/file=STC_RHO_v2_${EXP}.nc/append  transrho9n_above_rholevel[k=@sum],transrho9s_above_rholevel[k=@sum]
save/file=STC_RHO_v2_${EXP}.nc/append  transrho9n_int_above_rholevel[k=@sum],transrho9s_int_above_rholevel[k=@sum]
...
save/file=STC_RHO_9N9S_sections_${EXP}.nc/clobber TRANSRHO9N,TRANSRHO9S,TRANSRHO9N_INT,TRANSRHO9S_INT
