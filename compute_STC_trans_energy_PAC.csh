...
set RUN = (flux-only FAFHEAT FAFWATER FAFSTRESS FAFALL PASSIVEHEAT_FAFSTRESS)
##passive heat esta bloqueado-checar-21022022
set i = 1
...
let tempgrad_north_mean = tempgrad[x=@ave,y=10n:30n@ave]
let tempgrad_south_mean = tempgrad[x=@ave,y=10s:30s@ave]
let mek_10n       = ekman_trans[y=10n]
let mek_10s       = ekman_trans[y=10s]
let taux_10n      = taux[x=@ave,y=10n]
let taux_10s      = taux[x=@ave,y=10s]
let estc_10n      = estc_north[y=10n]
let estc_10s      = estc_south[y=10s]

save/file=STC_trans_energy_${EXP}_PAC.nc/append tempgrad_north_mean
save/file=STC_trans_energy_${EXP}_PAC.nc/append tempgrad_south_mean

save/file=STC_trans_energy_${EXP}_PAC.nc/append mek_10n
save/file=STC_trans_energy_${EXP}_PAC.nc/append mek_10s

save/file=STC_trans_energy_${EXP}_PAC.nc/append taux_10n
save/file=STC_trans_energy_${EXP}_PAC.nc/append taux_10s

save/file=STC_trans_energy_${EXP}_PAC.nc/append estc_10n
save/file=STC_trans_energy_${EXP}_PAC.nc/append estc_10s
