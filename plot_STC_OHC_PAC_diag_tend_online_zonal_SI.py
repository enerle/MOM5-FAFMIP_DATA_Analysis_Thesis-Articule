...
tend_name           = ["NET","(a) ADV","(b) EDDY","(c) ISO","vDIFF","srADV"]
tend_name_fafstress = ["NET","(d) ADV","(e) EDDY","(f) ISO","vDIFF","srADV"]
tend_name_fafwater  = ["NET","(d) ADV","(e) EDDY","(f) ISO","vDIFF","srADV"]
tend_name_fafheat   = ["NET","(g) ADV","(h) EDDY","(i) ISO","vDIFF","srADV"]
...
for i in range(len(exp)-1):
    ...
    cs  = plt.pcolormesh(lat2[i],depth2[i],advection[i][:,:]-advection[-1][:,:],shading='gouraud',cmap=cmap2)
    plt.clim(kmin,kmax)
    ...
    if i==0: plt.title(tend_name_fafstress[1])
    elif i==1: plt.title(tend_name_fafwater[1])
    elif i==2: plt.title(tend_name_fafheat[1])
    ...
    cs  = plt.pcolormesh(lat2[i],depth2[i],eddy[i][:,:]-eddy[-1][:,:],shading='gouraud',cmap=cmap2)
    plt.clim(kmin,kmax)
    ...
    if i==0: plt.title(tend_name_fafstress[2])
    elif i==1: plt.title(tend_name_fafwater[2])
    elif i==2: plt.title(tend_name_fafheat[2])
...
#    ax.set_ylabel('Depth [m]',fontsize=14)
    ...
    cs  = plt.pcolormesh(lat2[i],depth2[i],isopycnal_mix[i][:,:]-isopycnal_mix[-1][:,:],shading='gouraud',cmap=cmap2)
    plt.clim(kmin,kmax)
    ...
    if i==0: plt.title(tend_name_fafstress[3])
    elif i==1: plt.title(tend_name_fafwater[3])
    elif i==2: plt.title(tend_name_fafheat[3])
    ...
    plt.savefig('STC_OHC_PAC_diag_tend_online_zonal_' + exp[i] + '_SI.png',transparent = False,bbox_inches='tight',dpi=300)
...
ax = fig.add_subplot(2,3,1)###
cs  = plt.pcolormesh(lat2[-1],depth2[-1],advection[-1][:,:],shading='gouraud',cmap=cmap2)
plt.clim(kmin,kmax)
...
ax = fig.add_subplot(2,3,2)###
cs  = plt.pcolormesh(lat2[-1],depth2[-1],eddy[-1][:,:],shading='gouraud',cmap=cmap2)
plt.clim(kmin,kmax)
...
ax = fig.add_subplot(2,3,3)###
cs  = plt.pcolormesh(lat2[-1],depth2[-1],isopycnal_mix[-1][:,:],shading='gouraud',cmap=cmap2)
plt.clim(kmin,kmax)
...
#plt.show()
plt.savefig('STC_OHC_PAC_diag_tend_online_zonal_CTL_SI.png',transparent = False, bbox_inches='tight',dpi=300)
