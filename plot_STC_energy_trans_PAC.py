...
ax.plot(lat[-1],mek[-1],color=[.4,.4,.4],linestyle='solid',linewidth=2.) #MEk (Tau)
ax.plot(lat[i],10*(mek[0]-mek[-1]),color='red',linestyle='solid',linewidth=2.0) # MEk (F)
ax.plot(lat[i],mek[0],color='black',linestyle='solid',linewidth=2.0) #MEk (Tau+F)
...
plt.show()
#plt.savefig('ESTC_change-ekmantransport_PAC.png',transparent = True, bbox_inches='tight',dpi=600)

##tablas
print('-----------------------------------')
print('TABLES')
print('-----------------------------------')
print("& exp & $dTdy$  & $M_{Ek}$  & $Taux$  & $STC_E$ ")
for i in range(len(exp)-1):
    print("%s " %exp[i])
    print("\centering $North$" + " & %.2f " %((tgrad_north_mean[i]-tgrad_north_mean[-1])*1e6) + " & %.2f " %(trans_north_max[i]-trans_north_max[-1]) + " & %.2f " %((taux_north_max[i]-taux_north_max[-1])*1e3) + " & %.2f " %(estc_north_max[i]-estc_north_max[-1]))
    print("\centering $South$" + " & %.2f " %((tgrad_south_mean[i]-tgrad_south_mean[-1])*1e6) + " & %.2f " %(trans_south_max[i]-trans_south_max[-1]) + " & %.2f " %((taux_south_max[i]-taux_south_max[-1])*1e3) + " & %.2f " %(estc_south_max[i]-estc_south_max[-1]))
print("%s " %exp[-1])
print("\centering $North$" + " & %.2f " %(tgrad_north_mean[-1]*1e5) + " & %.2f " %trans_north_max[-1] + " & %.2f " %(taux_north_max[-1]*1e3) + " & %.2f " %estc_north_max[-1])
print("\centering $South$" + " & %.2f " %(tgrad_south_mean[-1]*1e5) + " & %.2f " %trans_south_max[-1] + " & %.2f " %(taux_south_max[-1]*1e3) + " & %.2f " %estc_south_max[-1])
...
for i in range(len(exp)-1):
    print("%s " %exp[i])
    print("\centering $North$" + " & %.1f " %(((tgrad_north_mean[i]/tgrad_north_mean[-1])-1)*100) + " & %.1f " %(((trans_north_max[i]/trans_north_max[-1])-1)*100) + " & %.1f " %(((taux_north_max[i]/taux_north_max[-1])-1)*100) + " & %.1f " %(((estc_north_max[i]/estc_north_max[-1])-1)*100))
    print("\centering $South$" + " & %.1f " %(((tgrad_south_mean[i]/tgrad_south_mean[-1])-1)*100) + " & %.1f " %(((trans_south_max[i]/trans_south_max[-1])-1)*100) + " & %.1f " %(((taux_south_max[i]/taux_south_max[-1])-1)*100) + " & %.1f " %(((estc_south_max[i]/estc_south_max[-1])-1)*100))
